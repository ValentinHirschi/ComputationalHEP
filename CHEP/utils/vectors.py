...
class LorentzVector(Vector):
    ...
    def __new__(cls, *args, **opts):
        if len(args) == 0:
            return super(LorentzVector, cls).__new__(cls, [0., 0., 0., 0.], **opts).view(LorentzVector)
        return super(LorentzVector, cls).__new__(cls, *args, **opts).view(LorentzVector)
...
class LorentzVectorList(list):
    ...
    def nice_string(self, n_initial=2):
        """Nice printout of the momenta."""
