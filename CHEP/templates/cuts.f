...
c DeltaR and invariant mass cuts
     ...
     $                          then
      ...
      function cosTheta(p1, p2)
          implicit none
          double precision cosTheta, p1(0:3), p2(0:3), p1_norm, p2_norm
          p1_norm = dsqrt(p1(1)*p1(1)+p1(2)*p1(2)+p1(3)*p1(3))
          p2_norm = dsqrt(p2(1)*p2(1)+p2(2)*p2(2)+p2(3)*p2(3))
          cosTheta = (p1(1)*p2(1)+p1(2)*p2(2)+p1(3)*p2(3))/(p1_norm*p2_norm)
          return
      end
...
c temporary variable for caching locally computation
      ...
      double precision cosTheta, cos42, cos43, Eg, THETACUT, EGCUT
      passcuts_pdgs = .true.
...
C
      if (p(0,4).ne.0.0d0) then
        Eg = p(0,4)
        cos42 = cosTheta(p(0,4),p(0,2))
        cos43 = cosTheta(p(0,4),p(0,3))
c        write(*,*) 'cosTheta(4,2)=',cos42
c        write(*,*) 'cosTheta(4,3)=',cos43
        THETACUT = 0.3d0
        EGCUT = 30.0d0
